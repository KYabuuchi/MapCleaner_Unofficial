...
private:
  ...
  void fillVector(grid_map::GridMap &grid, grid_map::Matrix &input_layer,
                  const grid_map::Index idx, std::vector<float> &cells) {
    grid_map::Index start_idx(idx[0] - (kernel_size_ / 2),
                              idx[1] - (kernel_size_ / 2));
    grid_map::Size kernel_size(kernel_size_, kernel_size_);
    for (grid_map::SubmapIterator it(grid, start_idx, kernel_size);
         !it.isPastEnd(); ++it) {
      if (0 > (*it)[0] || (*it)[0] >= input_layer.rows() || 0 > (*it)[1] ||
          (*it)[1] >= input_layer.cols()) {
        continue;
      }
...
public:
  ...
  bool compute(grid_map::GridMap &grid) {
    if (!grid.exists(input_layer_name_)) {
      RCLCPP_ERROR_STREAM(rclcpp::get_logger("median_filter"),
