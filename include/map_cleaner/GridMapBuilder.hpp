...
private:
  ...
  inline void getAABB2D(const CloudType &input, float &x_min, float &x_max,
  ...
  inline void updateCell(const PointType &p, float &cell_cnt, float &cell_avg,
                         float &cell_var, float &cell_min, float &cell_max,
                         float &cell_int) {
    if (!std::isfinite(cell_cnt)) // init
    {
      ...
      cell_var =
          (((cell_cnt * (cell_var + last_avg * last_avg)) + (p.z * p.z)) /
           (cell_cnt + 1.0)) -
          (cell_avg * cell_avg);
...
public:
  ...
  grid_map::GridMapPtr compute(const CloudType &cloud,
                               const PIndices &ground_indices) {
    ...
    grid_ptr->setGeometry(
        grid_map::Length(x_len, y_len), resolution_,
        grid_map::Position(x_len * 0.5 + x_min, y_len * 0.5 + y_min));
    ...
    for (size_t i = 0; i < ground_indices.indices.size(); i++) {
      ...
      grid_map::Index idx;
      if (!grid_ptr->getIndex(
              grid_map::Position(cloud[p_idx].x, cloud[p_idx].y), idx))
        continue;

      updateCell(cloud[p_idx], count_layer(idx[0], idx[1]),
                 average_layer(idx[0], idx[1]), variance_layer(idx[0], idx[1]),
                 min_layer(idx[0], idx[1]), max_layer(idx[0], idx[1]),
                 intensity_layer(idx[0], idx[1]));
    }
