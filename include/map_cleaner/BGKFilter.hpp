...
private:
  ...
  inline double kernel(const grid_map::Position &pos,
                       const grid_map::Position &center) {
    double d_r = (pos - center).norm() / kernel_radius_;
    double theta = 2.0 * M_PI * d_r;
    return (((2.0 + std::cos(theta)) / 3.0) * (1 - d_r)) +
           (std::sin(theta) / (2.0 * M_PI));
  }
  ...
  inline double computeBGK(const grid_map::GridMap &grid,
                           const grid_map::Position &center,
                           const grid_map::Matrix &input_layer) {
    int n = 0;
    double sum_k_z = 0.0;
    double sum_k = 0.0;
    for (grid_map::CircleIterator it(grid, center, kernel_radius_);
         !it.isPastEnd(); ++it) {
  ...
  inline double computeBilateralBGK(const grid_map::GridMap &grid,
                                    const grid_map::Position &center,
                                    const grid_map::Matrix &input_layer,
                                    const grid_map::Matrix &ref_layer) {
    int n = 0;
...
public:
  BGKFilter(const double kernel_radius, const double lambda, const double sigma,
            const int min_num, const std::string input_layer = "elevation",
...
#pragma omp parallel for
    for (int i = 0; i < rows; i++) {
      ...
      for (grid_map::SubmapIterator it(grid, start_idx, submap_size);
           !it.isPastEnd(); ++it) {
        grid_map::Position center;
        grid.getPosition(*it, center);
        first_pass_layer((*it)[0], (*it)[1]) =
            computeBGK(grid, center, input_layer);
      }
    }
    ...
    for (int i = 0; i < rows; i++) {
      ...
      for (grid_map::SubmapIterator it(grid, start_idx, submap_size);
        ...
        if (std::isfinite(input_layer((*it)[0], (*it)[1]))) {
          ...
          grid_map::Position center;
          grid.getPosition(*it, center);
          output_layer((*it)[0], (*it)[1]) =
              computeBilateralBGK(grid, center, input_layer, first_pass_layer);
        }
      }
    }
