...
private:
  ...
  void openEightNeighbor(std::vector<grid_map::Index> &open_list,
                         grid_map::Matrix &mask_layer,
                         const grid_map::Index &idx) {
    mask_layer(idx[0], idx[1]) = 1.0;
    for (int i = -1; i <= 1; i++) {
      for (int j = -1; j <= 1; j++) {
        grid_map::Index n_idx(idx[0] + i, idx[1] + j);
        if (0 > n_idx[0] || n_idx[0] >= mask_layer.rows() || 0 > n_idx[1] ||
            n_idx[1] >= mask_layer.cols()) {
          continue;
        }
  ...
  void computeMask(grid_map::GridMap &grid,
