...
public:
  ...
  virtual size_t loadFrameInfo(const std::string &pcds_dir,
                               const std::string &pose_file,
                               const int start = 0, const int end = -1) = 0;
...
public:
  ...
  size_t loadFrameInfo(const std::string &pcds_dir,
                       const std::string &pose_file, const int start,
    ...
    while (getline(ifs, csv_line)) {
      ...
      if (tmp >= start) {
        ...
        frame_filename_str << std::setfill('0') << std::right << std::setw(6)
                           << line_num << ".bin";
...
public:
  size_t loadFrameInfo(const std::string &pcds_dir,
    ...
    RCLCPP_INFO_STREAM(rclcpp::get_logger("data_logger"),
                       "Loaded: " << frame_info_buf_.size());
    return frame_info_buf_.size();
  }
  ...
  Frame loadFrame(const size_t idx) override {
    ...
    Frame frame;
    CloudType tmp_frame;
    pcl::io::loadPCDFile<PointType>(frame_info_buf_[idx].pcd_filename,
                                    tmp_frame);
    frame.idx = frame_info_buf_[idx].idx;
    frame.t = frame_info_buf_[idx].t;
    frame.r = frame_info_buf_[idx].r;
...
public:
  size_t loadFrameInfo(const std::string &pcds_dir,
    ...
    while (getline(ifs, csv_line)) {
      ...
      if (tmp >= start) {
        ...
        std::stringstream frame_filename_str;
        frame_filename_str << std::setfill('0') << std::right << std::setw(8)
                           << frame_info.idx << "/"; // frame_dir
        frame_info.pcd_filename = pcds_dir + frame_filename_str.str();
  ...
  void loadGLIMCloud(const std::string &frame_dir, CloudType &cloud) {
    { // read points
      cloud.clear();
      std::ifstream in(frame_dir + POINTS_FILENAME,
      ...
      while (in.good() && !in.eof() && cloud.size() < num_points) {
        ...
        if (!std::isfinite(eigen_p[0]) || !std::isfinite(eigen_p[1]) ||
            !std::isfinite(eigen_p[2]))
          is_dense = false;
    ...
    { // read intensities
      std::ifstream in(frame_dir + INTENSITIES_FILENAME,
                       std::ios::binary | std::ios::ate);
      if (!in.is_open())
        return;
