...
struct PointXYZ {
  ...
  PointXYZ(float _x, float _y, float _z, int _idx = -1)
...
struct Params {
  ...
  vector<long> num_sectors_each_zone;
  vector<long> num_rings_each_zone;
  ...
  Params() {
    ...
    num_lpr = 20; // Maximum number of points to be selected as lowest points
                  // representative.
    num_min_pts = 10; // Minimum number of points to be estimated as ground
                      // plane in each patch.
    ...
    RNR_ver_angle_thr =
        -15.0; // Noise points vertical angle threshold. Downward rays of LiDAR
               // are more likely to generate severe noise points.
    RNR_intensity_thr =
        0.2; // Noise points intensity threshold. The reflected points have
             // relatively small intensity than others.

    sensor_height = 1.723;
    th_seeds = 0.125;  // threshold for lowest point representatives using in
                       // initial seeds selection of ground points.
    th_dist = 0.125;   // threshold for thickenss of ground.
    th_seeds_v = 0.25; // threshold for lowest point representatives using in
    ...
    uprightness_thr = 0.707; // threshold of uprightness using in Ground
                             // Likelihood Estimation(GLE). Please refer paper
                             // for more information about GLE.
    adaptive_seed_selection_margin =
        -1.2; // parameter using in initial seeds selection

    num_sectors_each_zone = {16, 32, 54,
                             32};       // Setting of Concentric Zone Model(CZM)
    ...
    elevation_thr = {0, 0, 0,
                     0}; // threshold of elevation for each ring using in GLE.
                         // Those values are updated adaptively.
    flatness_thr = {0, 0, 0, 0}; // threshold of flatness for each ring using in
                                 // GLE. Those values are updated adaptively.
  }
};
...
public:
  ...
  PatchWorkpp(patchwork::Params _params) : params_(_params) {
    ...
    min_ranges_ = {params_.min_range, min_range_z2_, min_range_z3_,
                   min_range_z4_};

    ring_sizes_ = {
        (min_range_z2_ - params_.min_range) / params_.num_rings_each_zone.at(0),
        (min_range_z3_ - min_range_z2_) / params_.num_rings_each_zone.at(1),
        (min_range_z4_ - min_range_z3_) / params_.num_rings_each_zone.at(2),
        (params_.max_range - min_range_z4_) /
            params_.num_rings_each_zone.at(3)};
    ...
    std::cout << "PatchWorkpp::PatchWorkpp() - INITIALIZATION COMPLETE"
              << std::endl;
  }
...
private:
  ...
  void
  temporal_ground_revert(std::vector<double> ring_flatness,
